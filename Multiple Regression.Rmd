...
```
Initially, we were surprised with a R^2 value of .6982 by using all variables with no transformations. We regarded this value as significant and established .6982 as our baseline for improvement.
...
```
Since the p-value for the Shapiro-Wilk test was essentially 0, this indicated our model did not have mormal residuals, which confirms the information from the residual plot.
...
These tests and exploratory analysis of our inital linear regression model revealed to us we could not validate ANOVA assumption number 2 or 3. This led us to the conclussion that we needed to conduct a box-cox test and appropriately transform our target variable to account for these issues.
...
A GVIF score of ten or more indicates multicollinearity. None of the variables were close to these threshold, so multicollinearity was not present among our features.

#### Model Improvement

Now that we had analyzed our inital model and established areas of improvement, we then moved into refining our model. We first began with dropping variables that appeared to be insignificant according to high p-values. This included the variables: ROOMS, AYB, YR_RMDL, STORIES, and SALEMONTH. We began with the variable with the highest p-value and began dropping them from the model. If removing the variable decreased the R^2 value, we added the variable back and continued on to the next variable. The only positive result was dropping the variable AYB, which increased the R^2 score by .0002. Dropping ROOMS did not affect accuracy and removing the other variables lowered R^2. We decided to keep the other variables and only drop AYB, which resulted in a new R^2 baseline at .6984.

We also considered the variables with negative coefficients that we outlined previously. YR_RMDL was already tested because of it's high p-value but we tested dropping BEDRM as well. This resulted in a lower R^2, so we decided to keep the variable for the model. It could be the case, however, that in the region of this data set, having more bedrooms is undesirable past a certain point. This may be the case in a highly-populated with younger demographics.
...
We then progressed onto transforming the remaining variables. We conducted Box-Cox test on each predicitng variable to undertsand if there was a transformation that would somewhat normalize that variable. For some variables, we had to slightly manipulate them so they could processed (ex. added "1", Box-Cox cannot process variables with "0"). After analying the Box-Cox lambda values and applying the appropriate transformations, we added and removed those transformations until we recieved the highest R^2. Adding log(BEDRM + 1) and the log(GBA) brought our R^2 to .7269, which was an adequate improvement.

Next, we considered the target variable PRICE for transformation. According to our prelimminary tests, we could not conclude ANOVA assumptions two and three, that signaled to us that a target variable transformation maybe necessary. We conducted a BOX-Cox test and received the following output:
...
```
A lambda value of 0.33 is roughly half way in between the log and sqrt transformation values. We tested both transformations and considered the resdiual plot for each transformation. Log actually lowered the R^2 to .6589 while sqrt increased the R^2 to .7481 Both residual plots greatly improved the residual distribution. We decided on the sqrt transformation because of it's increase in R^2 accuracy:
```{r 5.8, echo=FALSE}
...
With an R^2 value of .7481, our model accounts for roughly 75% of variation in price from the predicting variables. This is the highest we were able to get the R^2 value following the process outlined above.
...
Y = -49280 + 56.94X1 + 35.80X2 + -38.24X3 + 112.40X4 + 60.61X5 + 3.52X6 + 1.44X7 + 25.00X8 + 14.45X9 + -99.70X10 + 403.7X11 + 85.75X12 + 65.30X13 + 14.83X12 + .049X13

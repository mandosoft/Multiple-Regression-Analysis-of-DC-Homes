...
#For many of these we had to factor the variables within the boxplot to have a more accurate look at where the outliers are. For example, because there are so many years, lumping them all together for AYB & YR_RMDL doesnt make sense.
...
corrplot(corr_matrix, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 36, insig = NULL)
...
We first began multiple linear regression with all the predictor variables utilized (except for SALEDATE). This gave us a baseline </sup> score to base feature transformation performance as well as any target variable transformations as well.
...
Now that we had analyzed our inital model and established areas of improvement, we then moved into refining our model. We first began with dropping variables that appeared to be insignificant according to high p-values. This included the variables: ROOMS, AYB, YR_RMDL, STORIES, and SALEMONTH. We began with the variable with the highest p-value and began dropping them from the model. If removing the variable decreased the R<sup>2</sup> value, we added the variable back and continued on to the next variable. The only positive result was dropping the variable AYB, which increased the R<sup>2</sup> score by .0002. Dropping ROOMS did not affect accuracy and removing the other variables lowered R<sup>2</sup>. We decided to keep the other variables and only drop AYB, which resulted in a new R<sup>2</sup> baseline at .6984.

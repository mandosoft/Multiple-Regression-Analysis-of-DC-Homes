...
#### Scatter Plots
We used a series of scatterplots for each variable compared to Price to look for linearity as well as how the independent variables affect the dependent. Similar to in the histograms, we saw little correlation with the SALEDAY and SALEMONTH variables that we created from SALEDATE. 
...
The ANOVA test also has three assumptions: All observations are independent of one another, the errors are normally distributed, and the variances of each group are equal. For the first assumption, we could resonably conclude that our observations were sampled independetly. Next, we decidied to check the second assumption of ANOVA: that all errors must be normally distributed. We plotted the residuals and annotated their distribution:
